...
export class CapabilityManagerComponent implements OnInit {
  ...
  setFormDefaults(): void {
    ...
    this.apiService.getCapOrgs(this.capability.ID).subscribe((data: any[]) => {
      this.orgRelations = [];
  ...
  submitForm() {
    ...
    if (this.capabilityForm.valid) {
      ...
      this.apiService.updateCapOrgs(this.capability.ID, this.capabilityForm.value).toPromise()
        .then(res => {
          // Grab new data from database
          this.apiService.getOneCap(this.capability.ID).toPromise()
